		...
		<b-form-row class="align-items-end">
			<b-col>
				<b-form-group label="Add a filter for the attribute" :label-for="uniqueId('select-attribute')">
					<v-select :id="uniqueId('select-attribute')" v-model="selectedAttribute" :options="attributeOptions" label="verbose_name"></v-select>
				</b-form-group>
			</b-col>
			<b-col cols="auto" class="mb-3">
				<b-button variant="light" :disabled="!selectedAttribute" :title="selectedAttribute ? `Add filter for attribute ${selectedAttribute.verbose_name}` : 'Select an attribute to add a filter'" size="sm" @click="addAttributeFilter"><b-icon icon="plus"></b-icon></b-button>
			</b-col>
		</b-form-row>
		<b-form-row v-for="attribute in attributeFilters" :key="attribute.name" class="align-items-top">
			<b-col>
			...
			<b-col cols="auto" class="mb-3">
				<b-button variant="light" :title="`Remove filter for attribute ${attribute.verbose_name}`" size="sm" @click="removeAttributeFilter(attribute)"><b-icon icon="x"></b-icon></b-button>
			</b-col>
		</b-form-row>
	</div>
</template>
