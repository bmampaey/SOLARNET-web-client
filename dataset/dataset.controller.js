...
function($scope, Dataset, Telescope, Characteristic, Tag, $uibModal, bsLoadingOverlayService) {
	...
	console.log("DatasetController scope", $scope.$id);
	...
	$scope.create_user_data_selection = function(selectedDatasets)
	{
		console.log('Adding to user_data_selection', selectedDatasets);
	};
...
function($scope, $uibModalInstance, dataset) {
	console.log("DatasetDetailController scope", $scope.$id);
...
function($scope, Metadata, Tag, METADATA_SEARCH_PARAMS, bsLoadingOverlayService) {
	console.log("MetadataController scope", $scope.$id);
	...
	$scope.search_metadata = function() {
		...
		search_params = {dataset: $scope.dataset.id + '_metadata'};
