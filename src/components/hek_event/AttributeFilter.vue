<template>
	<div>
		<b-form-row>
			<b-col>
				<b-form-group label="Add a filter for the attribute" :label-for="uniqueId('select-attribute')">
					<b-form-select :id="uniqueId('select-attribute')" v-model="selectedAttribute"
						:options="attributeOptions"></b-form-select>
				</b-form-group>
			</b-col>
			<b-col cols="auto" align-self="end" class="mb-3">
				<b-button variant="light"
					:title="selectedAttribute ? `Add filter for attribute ${selectedAttribute.name}` : 'Select a attribute to add a filter'"
					@click="addAttributeFilter"><b-icon icon="plus"></b-icon></b-button>
			</b-col>
		</b-form-row>
		<b-form-row v-for="attribute in attributeFilters" :key="attribute.name">
			...
			<b-col cols="auto" align-self="end" class="mb-3">
				<b-button variant="light" :title="`Remove filter for attribute ${attribute.name}`"
					@click="removeAttributeFilter(attribute)"><b-icon icon="x"></b-icon></b-button>
			</b-col>
		</b-form-row>
	</div>
</template>
